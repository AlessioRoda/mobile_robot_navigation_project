...
def clbk_pos(pos):
    ...
    desired_position_.x=pos.position.x
    desired_position_.y=pos.position.y
    desired_position_.z=pos.position.z
...
def clbk_laser(msg):
    ...
    #}
    regions_ = {
        'right':  min(min(msg.ranges[0:43]), 10),
        'fright': min(min(msg.ranges[44:87]), 10),
        'front':  min(min(msg.ranges[88:131]), 10),
        'fleft':  min(min(msg.ranges[132:175]), 10),
        'left':   min(min(msg.ranges[176:219]), 10),
    }
...
def main():
    ...
    sub_pos= rospy.Subscriber('/move_base_simple/goal', PoseStamped, clbk_pos)
